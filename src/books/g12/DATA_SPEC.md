# Authoring spec — Emar English Series, Grade 12, Scientific Section

The app turns this textbook into a Duolingo-style course for Syrian students in Grade 12 (Scientific Section).
...
1. **Never leave the curriculum.** Every word, text, rule, dialogue and sentence comes from the book's own pages.
   The verbatim text of the pages is in `book-source/g12/moduleN.md`; the page images are listed in your task.
...
## Checking
`node scripts/check-book.mjs g12 N` must print `OK`. It verifies counts, answer indexes, distinct options, every Arabic
field, and that every example, reading paragraph, dialogue line and sentence exists in `book-source/g12/moduleN.md`.

## Grade 12 specifics — this is the Baccalaureate year
The students sit the national Baccalaureate exam on this book. The final paper has: a reading passage with
comprehension questions (true/false, answer-the-question, MCQ), vocabulary from the units (meanings, phrasal verbs,
idioms, word families / derivatives), grammar (tenses, reported speech, conditionals, passive and causative,
paired conjunctions, inversion, question tags — whatever the book teaches), pronunciation (the book's sounds,
stress and intonation points), spotting the wrong part of a sentence, and a composition of about 80–100 words.
So: make the grammar and vocabFocus drills cover every form the book teaches, include "rewrite / choose the
